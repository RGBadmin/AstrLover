...
class PresenceCore:
    ...
    def _schema_defaults() -> dict:
        ...
        try:
            # 本文件在 astrlover/presence/ 下，配置模板在插件根目录
            raw = (Path(__file__).resolve().parents[2] / "_conf_schema.json").read_text(encoding="utf-8")
            walk(json.loads(raw))
    ...
    def _flatten_conf(config) -> dict:
        ...
        填过，那当然以新的为准。
        """
        flat = PresenceCore._walk_conf(config, {})
        stale = {
            k: v
        ...
        if not stale:
            return flat
        defaults = PresenceCore._schema_defaults()
        kept = [
    ...
    def _split_query(keywords: str, cap: int = 16) -> list[str]:
        ...
        for w in raw:
            ...
            pieces = PresenceCore._seg(w) if cjk_long else []
    ...
    def _desc_segments(descr: str) -> dict[str, str]:
        ...
        全文向量，行为跟以前一样。
        """
        layers = PresenceCore._desc_layers(descr)
        if not layers:
            return {}
        ...
        if (tag := PresenceCore._tag_line(descr)) and tag not in out.get("vec_act", ""):
    ...
    def _vision_payload(cfg: dict, mime: str, b64: str) -> dict:
        ...
        if fmt == "anthropic":
            ...
            body = {
                ...
                "generationConfig": PresenceCore._gemini_gen_config(cfg),
            }
            if safety := PresenceCore._gemini_safety(cfg):
