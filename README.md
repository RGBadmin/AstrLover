...
| 生命档案 | 人格、外观、身世、社交圈、作息的 YAML 档案，表现由人格推导 | Web 面板 |
...
最小可用只需要一步：填「恋人的 Telegram 数字 user id」，她就带着人格活起来了。其余按需开。
...
**词面**：按 IDF 加权打分。不取交集——一句「酒店里穿灰丝踩红底细高跟」拆出六七个词，只要一个词对不上（你说「足底」而档案里写「脚底」）交集就空了，整句话什么都搜不到。打分则是漏词只降排名，不至于让图消失。整词一张都没匹配上时会拆成单字重试。
...
`data/plugin_data/astrlover/persona/profile.yaml`，首次启动自动从模板生成，Web 面板可直接编辑并热加载。包含：身份、外观基准、性格与说话风格、身世条目、3~5 个固定 NPC 社交圈、作息与活动池、关系与称呼。

**想要她更粘人、更毒舌、更文静——改档案的性格描述。** 一切表现由人格推导，系统不设「粘人度」这类数值旋钮。

系统演化出来的状态（关系阶段、她换过的签名、临场编过的设定）写在 `dynamic.yaml`，换档案不丢「你们的过去」。她临场编的细节（「我妈是老师」）会立刻固化成永久事实，之后永远一致——下次说妈妈是医生就穿帮了。
...
她的负面情绪（委屈、吃醋）零后果自动消散：不哄也会自己好，哄一句立刻雨过天晴；表达永远是「可爱」而非「指责」。冷暴力、不回消息作为惩罚、必须道歉才恢复、翻旧账、制造愧疚——这些是写死的硬约束，档案不可覆盖。
...
AstrBot WebUI → 插件 → AstrLover → Pages：总览（此刻 / 心情 / 日程 / 模块健康）、生命档案编辑（热加载）、日记周记、记忆（小抄 + 事实）、对话素材、生活事件流、排期管理、一键导出档案与记忆包。

面板经 Dashboard 登录鉴权，等同最高权限，**切勿把 Dashboard 暴露公网**。重交互（相册索引、让她说话）在控制台 bot 里做，面板负责"看"和"编辑档案"。
...
├── persona/profile.yaml    生命档案（你写的静态基线）
