...
- **其余全部在插件自己的 UI 里**（WebUI → 插件 → AstrLover → 打开页面 →「设置」）：视觉解析、相册、图片记忆、动态、头像签名、主动消息、生图、轻量模型、向量模型、语音，共 68 项，分组排列，改完即时生效不用重载。视觉和向量那两组旁边直接有「测一下」按钮——调完当场验证。
...
- **设置**：68 项，分组排列、改完即时生效、视觉与向量旁边有「测一下」。
