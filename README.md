...
- **AstrBot 插件配置页**只有 6 项「接线」——恋人 user id、控制台 Bot Token 与管理员 ID、Embedding / TTS 的 Provider ID、生命层总开关。装机时填一次，之后不用再碰。
- **其余全部在插件自己的 UI 里**（WebUI → 插件 → AstrLover → 打开页面 →「设置」）：视觉解析、相册、图片记忆、动态、头像签名、主动消息、生图、语音，共 57 项，分组排列，改完即时生效不用重载。视觉和向量那两组旁边直接有「测一下」按钮——调完当场验证。
...
几条经验：**别写成助手腔**（「我可以帮你…」）；把口癖、雷区、说话习惯写具体（模型很吃这个）；NPC 的名字与设定一次写死（下次变成小美就穿帮）；把作息写清楚（「工作日早 8 点起，周日单休」——她排日程时要用）。
...
| 记录 | 谁创建 | 什么时候消失 |
|---|---|---|
| 日程 | 她每天自己排（人格在上下文里，她知道自己几点起、哪天休息） | 过期清理 |
...
**作息为什么不是配置项**：每个人写人设的方式都不一样，从自由文本里稳定提取不出来；就算提取出来，你一改人设它就成了过期数据。所以改成**她每天自己排一次明天的日程**——排出来的是记录，排得不对你直接改那条，人设改了第二天自动跟上。心跳只读记录：没有记录就不做作息假设（当她随时在线）。
...
AstrBot WebUI → 插件 → AstrLover → Pages：总览（此刻 / 心情 / 日程 / 模块健康）、**设置**（57 项，分组、即时生效、就地测试）、记录页（分类浏览 + 增删改）、日记周记、记忆（小抄 + 事实）、对话素材、生活事件流、排期管理、一键导出记忆包。
...
> v1.0 之前不做数据迁移：升级后如果表结构变了，删掉 `astrlover.db` 重来即可（相册要重新 scan + index）。上万张图的描述几百 MB 很正常，向量在 `vec/` 下另算。
...
- `astrlover.db`：全部记录（日程/事实/日记/事件/纪念日）一起没，她等于失忆重来；人设在 AstrBot 人格里，不受影响
