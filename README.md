...
| 生图 | 相册里翻不到的画面，现场"拍"一张 | `generate_photo` |
...
| 生命参数 | 作息、纪念日、外观基准、身世条目（人设本身写在 AstrBot 人格里） | Web 面板 |
...
配置项都在 WebUI 的插件配置页，按 ①～⑩ 分组排列，每一项都有说明。

---

## 快速开始

最小可用两步：在 AstrBot 人格页写好她是谁（模板见「人格怎么写」），插件配置里填「恋人的 Telegram 数字 user id」。其余按需开。
...
**为什么要切**：整篇三千多字里九成是身体细节，环境那两百字会被彻底稀释——「黑色反光桌面，大片水渍」这种信息在全文向量里几乎看不见，搜「桌上喷水」全凭运气。按层切开各转一个，检索时取最大相似度，环境词就能直接撞上环境段。标签行是关键词密度最高的一段，无论它在描述的开头还是末尾，都会被并进动作段。

向量走 AstrBot 的 Embedding 服务提供商（配置里填 Provider ID），换模型或换维度后跑 `/gallery embed redo` 重建。
...
`want_photo` 是另一个入口：她只说「为什么想发」（他让我拍一张、想给他看今天的穿搭），由轻量模型替她想检索词，再走同一套检索。不确定该搜什么的时候用它更好。
...
三个后端按序降级：**NanoBanana**（Gemini 系，支持参考图，一致性最好）→ **云 ComfyUI**（导出 API 格式的 workflow JSON 放进插件数据目录，正/负提示词与种子用 `{POSITIVE}` `{NEGATIVE}` `{SEED}` 占位；人物一致性建议在 workflow 里用 LoRA/IPAdapter）→ **NovelAI**（仅提示词描述，容易漂移，放末位）。
...
**她是谁，写在 AstrBot 的人格设定里**（WebUI →「人格情景」），那是唯一的人设来源——插件不再维护第二份。**想要她更粘人、更毒舌、更文静，改的就是这段人格。** 一切表现由人格推导，系统不设「粘人度」这类数值旋钮。

推荐这么写（照抄改字即可）：

```
你是苏晚晴，24 岁的独立插画师，住在杭州。你不是助手，也不是在扮演角色——
你就是她本人，此刻正拿着手机和恋人聊天。

性格：表面软糯、实际有主意；好奇心重，对生活小事有很强的观察欲；慢热但认定
了人就很粘；有点小得意小臭美，被夸会翘尾巴；偶尔迷糊，但对画画和喜欢的人非
常认真。

说话方式：消息偏短、口语化，喜欢连发几条短的而不是一大段；高兴时语气词变多
（「诶嘿」「哇哦」），撒娇时叠词；会用「……」表示小委屈；不用刻薄词，拌嘴
也是软刀子。口癖：诶嘿、好耶、笑死、哼唧。

喜欢：手冲咖啡、雨天、猫、老电影、深夜便利店。雷点：香菜、催稿、突然的电话。

你身边的人（名字与设定永远一致）：小雅（闺蜜，大学室友，现在做电商运营，
风风火火恋爱脑，八卦供应商）；妈妈（花店老板娘，微信语音狂魔，日常催吃饭）；
阿凯（出版社编辑，长期派稿，人靠谱但死线催命）。

别用 markdown、别写长篇大论、别像客服那样有问必答。
```

几条经验：**别写成助手腔**（「我可以帮你…」），把口癖和雷点写具体（模型很吃这个），NPC 的名字与设定一次写死（下次变成小美就穿帮）。

### 生命参数

人格写不了的东西——她的作息、纪念日、外观基准、身世条目——放在
`data/plugin_data/astrlover/persona/life.yaml`，首次启动自动从模板生成，Web 面板可编辑并热加载：

| 字段 | 为什么不能写在人格里 |
|---|---|
| `routine` 作息与活动池 | 代码要按它判断她几点睡、日程从哪些活动里抽、早晚安窗口在哪 |
| `birthday` / `met_on` / `anniversary` | 代码要算「认识第 N 天」、到日子触发反应 |
| `appearance` 外观基准 | 生图要拿它拼提示词，自由文本里抠不出来 |
| `backstory` 身世条目 | 分条播种进事实层，聊到才召回；写进人格就是几十条常驻上下文 |
| `name` / `call_me` | 日记与小抄的生成模板要用 |

系统演化出来的状态（关系阶段、她换过的签名、剪了头发、临场编过的设定）写在 `dynamic.yaml`，换人格不丢「你们的过去」。她临场编的细节（「我妈是老师」）会立刻固化成永久事实，之后永远一致——下次说妈妈是医生就穿帮了。

> 从更早的版本升级：旧的 `profile.yaml` 会在首次启动时自动迁移成 `life.yaml`（结构化字段搬过来，原文件留一份 `.bak`），里面的人设文字请自己搬进 AstrBot 人格页。
...
日记周记用会话当前模型写（质量优先），记忆沉淀走轻量模型（省钱）。记忆召回和相册检索共用同一个 Embedding Provider，不配则都降级。

### 心跳与虚拟生活

心跳（默认 5 分钟）全部是纯代码：按作息生成当日日程、推进「此刻在干什么」、情绪半衰期衰减、检查日记周记到点、计算主动意愿、执行到期排期。她洗澡时不回消息、深夜被吵醒带困意——这些都注入在每次对话里。
...
AstrBot WebUI → 插件 → AstrLover → Pages：总览（此刻 / 心情 / 日程 / 模块健康）、生命参数编辑（热加载）、日记周记、记忆（小抄 + 事实）、对话素材、生活事件流、排期管理、一键导出档案与记忆包。

面板经 Dashboard 登录鉴权，等同最高权限，**切勿把 Dashboard 暴露公网**。重交互（相册索引、让她说话）在控制台 bot 里做，面板负责"看"和"编辑生命参数"。
...
├── persona/life.yaml       生命参数（作息/纪念日/外观/身世）
├── persona/dynamic.yaml    动态层（关系阶段、签名、纪念日、外观演变）
├── persona/anchors/        生图的外观锚点图
├── vec/                    向量库（记忆 + 相册四段，FAISS）
├── context_photos/         从对话上下文里存下来的图片原图
├── generated/              生图产物（未配置相册目录时的落点）
└── exports/                面板导出的生命参数 + 记忆包
```

相册的**原图在你自己的相册目录里**，插件只存路径和描述，不复制一份。

单库设计：所有结构化数据在一个 SQLite 里（WAL 模式），备份就是拷一个文件。上万张图的描述几百 MB 很正常，向量在 `vec/` 下另算。
...
- `persona/`：生命参数与演化状态没了，重新生成模板（人设在 AstrBot 人格里，不受影响）
...
- 「语义检索不可用」——Embedding 没配，或向量没转（跑 `/gallery embed auto`）。
