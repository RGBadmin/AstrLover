...
├── settings.py          设置 SPEC（74 项）+ Settings：接线取配置页，其余取库
...
决定"插件怎么找到你和你的服务"，装机设一次。其余 74 项在 settings.py
...
还是模型名错。存储仍用 AstrBot 内置的 FaissVecDB（那是库，不是耦合），
客户端鸭子类型对齐它要的四个方法，不继承 AstrBot 基类。
模型或维度一变，`vec/embed_meta.json` 里的签名就对不上，两个向量库
一起清空重建——旧向量是另一个空间的坐标，混着检索不报错，只会安静地搜错。
...
`python -m pytest tests` —— 214 项，不需要安装 AstrBot：
